...
drawbar(Monitor *m)
{
 	int x = 0, w, tw = 0, moveright = 0, stw = 0;
 	...
 	for (i = 0; i < strlen(barlayout); i++) {
 		switch (barlayout[i]) {
          ...
          tw = TEXTW(stext);
 					if (moveright) {
 						x -= tw;
            // drw_text(drw, x - stw, 0, tw, bh, lrpad / 2 - 2, stext, 0);
        		drw_text(drw, m->ww - tw, 0, tw, bh, lrpad / 2, stext, 0);
 					} else
 						x = drw_text(drw, m->ww - tw, 0, tw, bh, lrpad / 2, stext, 0);
 				}
       	resizebarwin(m);
 				break;
            ...
            tagtext = occ & 1 << j ? alttags[j] : tags[j];
            tw += TEXTW(tagtext);
 						// tw += TEXTW(tags[j]);
 					}
 					x -= tw;
          ...
          tagtext = occ & 1 << j ? alttags[j] : tags[j];
 					...
 					drw_text(drw, x, 0, w, bh, lrpad / 2, tagtext, urg & 1 << j);
 					if (occ & 1 << j)
